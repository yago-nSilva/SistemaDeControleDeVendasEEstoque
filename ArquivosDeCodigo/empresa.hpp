...
{
public:
  // Funções para exibir nomes de usuário e senhas
  void showPasswords();
  void showUsernames();

  // Funções para inserir nomes de usuário e senhas
  void insertUsername(std::string username);
  void insertPassword(std::string password);

  // Funções para excluir o último nome de usuário e senha cadastrados
  void deleteUsername();
  void deletePassword();

  // Função para confirmar uma senha
  bool confirmPassword(std::string confirmation_password);

private:
  // Vetores para armazenar os nomes de usuário e senhas
  std::vector<std::string> Username;
  std::vector<std::string> Password;
};
...
{
public:
  // Função para transformar o nome do arquivo
  void turnintoNamefile(std::string *pointerto_namefile, std::string namefile);

  // Funções virtuais puras relacionadas à manipulação de arquivos
...
public:
  ...
  // Implementação da função para escrever em um arquivo de login
  virtual bool writeonFile(std::string username, std::string password) override {
    ...
    do {
      if (_file.eof()){
        ...
        _file << std::endl;
        _file << "User: " << username << " // " << std::endl
              << "Senha: " << password << std::endl << marker_;
      }
  ...
  // Implementação da função para verificar um arquivo de login
    ...
    while (getline(_file, line)){
      if (line == username_string){
        while (line == username_string){
          *search_username_ptr = "";   // Resetar o search_username;
  ...
  // Implementação da função para pesquisar em um arquivo de login
...
{
public:
  // Verifica se o arquivo de usuário existe
  bool doesuserfileExists(std::string const namefile);

  // Implementação da função para criar um arquivo de usuário
  virtual bool createFile (std::string const namefile) override {
  ...
  // Implementação da função para pesquisar em um arquivo de usuário
  virtual bool searchonFile(std::string username, std::string password) override {
